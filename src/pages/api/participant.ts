...
const handleDelete = (req: NextApiRequest, res: NextApiResponse<Response>) => {
  const { id } = req.body
  const data = handleGet(req, res)
  const participantIndex = data.participants.findIndex((participant: Participant) => participant.id === id)
...
const handlePut = (req: NextApiRequest, res: NextApiResponse<Response>) => {
  ...
  const participantIndex = data.participants.findIndex((participant: Participant) => participant.id === id)
  if(participantIndex >=0) {
    data.participants[participantIndex].paid = !data.participants[participantIndex].paid
    fs.writeFileSync(filePath, JSON.stringify(data))
  }
  return res.status(200).json(data)
