...
export const NewBarbecueEvent = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className='flex flex-col  justify-center p-4 rounded-sm shadow-md bg-white'>
      ...
      <DatePicker
        className='border border-amber-300 mb-9'
        minDate={today}
        selected={eventDate}
      ...
      <input
        ...
        className='mb-9 border border-amber-300'
      />
      <label htmlFor='valorSugerido'>Valor sugerido</label>
      <input
        onChange={(e) => setSuggestedValue(parseInt(e.target.value))}
        value={suggestedValue}
        type='number'
        placeholder='000'
        className='mb-9 border w-20 border-amber-300'
      />
      ...
      <div className='flex justify-between mt-10'>
        <button
          type='button'
          className='bg-red-500 text-white rounded-md w-36'
          onClick={() => onClose()}
        >
          Cancelar
        </button>
        <button
          ...
          className={classNames(
            'ml-2 p-3 bg-black text-white rounded-md w-36',
            {
