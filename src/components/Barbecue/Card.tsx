...
export default function Card({
  ...
  const participants = participantState.participants?.filter(
    (participant) => participant.barbecueId === id,
  );

  const food = participants?.reduce(
    (acc, participant) => acc + participant.amount,
    0,
  );
  const drink = participants?.reduce(
  ...
  const handleDeleteEvent = async () => {
    const response = await fetch(
      process.env.NEXT_PUBLIC_API_URL + `/barbecue`,
      {
        method: 'DELETE',
        headers: {
  ...
  return (
    <div className='flex flex-col px-3 py-2 shadow-lg justify-center bg-white'>
      ...
      <div className='flex justify-between mt-6 items-center w-full'>
        <button
          type='button'
          className='flex items-center justify-between w-8 group'
          onClick={onClick}
        >
          <ParticipantIcon />
