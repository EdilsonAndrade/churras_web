...
export const DetailPage = () => {
  ...
  const barbecueParticipants = participants.filter(participant => participant.barbecueId === barbecueId)
  const food = barbecueParticipants.reduce((acc, participant) => acc + participant.amount, 0)
  const drink = barbecueParticipants.reduce((acc, participant) => acc + participant.drink, 0)
  ...
  return (
    <main
      className="relative mt-20 flex min-h-screen flex-col items-center pt-10 bg-white"
    >
      ...
      {barbecue && barbecueId
        ...
        <div className="flex flex-col justify-between shadow-lg px-3 py-2 bg-white absolute -top-10">
          ...
          <div className="w-full flex justify-between" onClick={handleBack}>
            ...
            <div className="w-20">
              <div className="flex items-center"><ParticipantIcon /> <span className="ml-2">{participants.length}</span></div>
              <div className="flex items-center"><MoneyIcon /><span className="ml-2">{total}</span></div>
              <div className="flex items-center"><Image src="/assets/beer.png" width={20} height={16} alt="drink" /><span className="ml-2">{drink}</span></div>
            </div>
          </div>
          <div className="flex flex-col">
            ...
            <div className="flex flex-col justify-between">
              {barbecueParticipants.map(participant => {
                ...
                return (
                  <ParticipantList id={id} key={id} name={name} paid={paid} amount={amount} drink={drink} />
                )
              })}

            </div>

            <ParticipantForm barbecueId={barbecueIdString} suggestedValue={barbecue.suggestedValue || 0} />
          </div>
        </div>
        :
        <>
          <button onClick={handleBack} type="button"><ArrowLeftCircle size={40} /></button>
